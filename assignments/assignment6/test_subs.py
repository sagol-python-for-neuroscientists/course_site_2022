...
class TestGamePieces:
    ...
    def test_pieces_general_constant_outline(self, plane):
        ...
        np.testing.assert_array_equal(
            np.array([[1]], dtype=np.int16), g.outline
        )
...
class TestSubmarinesGame:
    ...
    def test_game_coord_in_board_valid_coord(
        self, submarines_game, inp, truth
    ):
    ...
    def test_game_too_small_of_a_board(self):
        with pytest.raises(AssertionError):
            SubmarinesGame(
                pieces={SubmarinesPieces.GENERAL: 1}, board_shape=(0, 0, 3)
            )

    def test_game_too_small_of_a_board_incompatible_num_layers(self):
        with pytest.raises(AssertionError):
            SubmarinesGame(
                pieces={SubmarinesPieces.GENERAL: 1}, board_shape=(5, 5, 2)
            )
